/* Serverless platform entry for the Deepseek debate-partner proxy (Step 8.3).

   Deploy-time wrapper only — all logic lives in (and is unit-tested from)
   src/services/debate/deepseekHandler.ts. Intentionally OUTSIDE src/, so it is
   ...
   DEEPSEEK_API_KEY is read from the server environment here and never reaches
