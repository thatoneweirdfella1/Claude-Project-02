/* Serverless platform entry for the Google debate-partner proxy (Step 8.3).

   Deploy-time wrapper only — all logic lives in (and is unit-tested from)
   src/services/debate/googleHandler.ts. Intentionally OUTSIDE src/, so it is
   ...
   GOOGLE_API_KEY is read from the server environment here and never reaches
   the browser (ROUTING.md DEBATE PARTNERS: "Each provider's API key is a
   separate secret... Not committed to the repo, ever."). */

import { handleGoogleRequest } from "../src/services/debate/googleHandler";
