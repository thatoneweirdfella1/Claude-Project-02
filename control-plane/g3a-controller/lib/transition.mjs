...
const TRANSITIONS = Object.freeze({
  "Open": ["Leased", "Blocked — user decision required"],
  "Leased": ["In progress", "Open"],
  "In progress": ["Self-check passed", "Open", "Blocked — user decision required"],
  "Self-check passed": ["Awaiting independent audit"],
  "Awaiting independent audit": ["Failed", "Independently verified"],
  "Failed": ["Correction in progress"],
  "Correction in progress": ["Self-check passed", "Failed", "Open"],
  "Independently verified": ["Accepted"],
  "Accepted": [],
  "Blocked — user decision required": ["Open"]
});
...
export function transition({ task, to, actor, attestation = null, trustedValidation = false, decisionQueue = [] }) {
  ...
  if (to === "Awaiting independent audit" && actor.principal !== task.author_principal) throw new TransitionError("Only the author worker may submit its self-check for audit");
...
export function unlockableTasks(tasks) {
  return Object.entries(tasks).filter(([, task]) => task.state === "Open" && task.dependencies.every(id => tasks[id]?.state === "Accepted")).map(([id]) => id);
}
