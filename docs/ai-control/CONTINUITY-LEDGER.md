...
### CL-0058 — 2026-09-09 UTC — signed-delivery mismatch proven and login-free App repair self-checked

- **Actor:** OpenAI Codex G3-A continuation author.
- **Task/phase:** G2 autonomy correction / G3-A genuine signed-webhook and duplicate-delivery proof.
- **Repository/branch/starting commit:** Existing `divergence/reliability-staging` at remotely confirmed trigger `269afa2b3810543a3807f26cea986984096168db`; clean detached worktree; no branch created.
- **Authority/source:** User instructed Codex to finish G3-A and explicitly prohibited repeating GitHub password login because the account does not support it. D-016, D-017, CURRENT-TASK, and the G3-A contract remain controlling.
- **Action and affected files:** Read the complete mandatory controls and verified every manifest hash. Vercel logs for exact deployment `dpl_8N5fdgtDovjrrjyHf8e6BubmqALM` showed genuine Hookshot push delivery `4010aa1e-ac0b-11f1-9a8b-b30352f0b612` reached the corrected raw-body handler with a signature and 9,409-byte JSON body but failed signature validation. Added an authenticated maintenance endpoint and App-JWT host methods that can set GitHub's webhook secret equal to Vercel's existing secret and request redelivery of the latest successful push without user-account login or secret disclosure.
- **Reason and rejected alternatives:** The safe metadata proves a configuration mismatch; more raw-body changes would not repair it. Official GitHub API documentation requires an App JWT for webhook configuration and redelivery. Rejected password login, App private-key rotation, displaying or moving stored secrets, manual GitHub UI editing, and weakening signature verification.
- **Command/test/check and actual result:** Controller package tests pass 14/14; combined G3-A tests pass 35/35; course-control tests pass 61/61; diff whitespace passes. Hostile tests prove App-JWT use rather than an installation/user token, unauthenticated synchronization rejection, and exact action routing.
- **Evidence/artifact/hash:** E-052; GitHub App ID `4879098`; trigger `269afa2b3810543a3807f26cea986984096168db`; exact deployment and delivery GUID above.
- **Failure/correction/uncertainty:** The repair is not yet published, deployed, or invoked. Genuine signed success and duplicate refusal remain Open. Worker URLs remain absent. No activation, acceptance, independent audit, merge, ruleset, product, or private-key change occurred.
- **Resulting status/gate change:** Mismatch diagnosis and login-free repair are author Self-check passed. G3A-01 live success/duplicate proof remains Open. `SAFE TO SWITCH: NO` during publication and live repair.
- **Exact next action:** Publish and deploy this checkpoint, set a fresh controller-only `BOOTSTRAP_SECRET`, invoke secret synchronization, generate one signed staging push, request App-authenticated redelivery, verify duplicate refusal, and publish final synchronized evidence.

### CL-0059 — 2026-09-22 UTC — G2 publication enforcement completed and two prior overclaims retracted

- **Actor:** Claude Code G2 continuation author (same author lineage as CL-0043; not an independent reviewer).
- **Task/phase:** G2 autonomy correction / publication-enforcement completion and correction-distinction test coverage.
- **Repository/branch/starting commit:** Existing `divergence/reliability-staging` at `2a6434d8f13af23dfa3787d58c8dd33e82035ff4`.
- **Authority/source:** User's approved plan to finish the three defects left behind by CL-0043, bounded to G2 `allowed_paths`. D-016 supersedes routine user acceptance, so no user-acceptance mechanism was built.
- **Action and affected files:** Removed the dead duplicate `validatePublicationPreflight` from `scripts/ai-control-state.mjs` (unreferenced repo-wide; it never compared hashes, only checked that checksum targets existed). Added the two checks requirement #3 named but `verifyPublicationPreflight` lacked — authorized working branch and expected remote parent — in `scripts/ai-course-control.mjs`. Wired the preflight into `.github/workflows/ai-course-control.yml` for pushes to the working branch and declared that file in `control_change.paths`. Added four correction/contamination tests to `scripts/ai-course-control.test.mjs`. Synchronized `CONTROL-STATE.json`, `HANDOFF.md`, `EVIDENCE-INDEX.md`, `GATE-STATUS.md`, and `SHA256SUMS`.
- **Reason and rejected alternatives:** CL-0043 recorded two claims that were not true. It reported "publication-preflight passes" after running the dead duplicate, and it reported hostile-test coverage for the correction/contamination distinction when the suite count was unchanged and the new `edge.type !== "correction"` branch had no test at all. Both are retracted in E-038. Rejected leaving the dead duplicate in place (it is precisely what produced the false pass), and rejected implementing user-acceptance enforcement (prohibited by D-016).
- **Command/test/check and actual result:** Course-control tests 61 → 65, all pass. The two discriminating tests were confirmed to FAIL against the pre-change logic (correction removed from `DEPENDENCY_TYPES` and the propagation guard reverted) and to pass with it; the two guard tests hold both ways by design. `sha256sum -c` passes for every recorded file. Running the completed preflight detected that `2a6434d` recorded `269afa2` as its confirmed checkpoint while its real parent is `703106e`; 3 of the 4 preceding commits match the base-recording convention, so this checkpoint repairs that one stale record.
- **Evidence/artifact/hash:** E-053; retractions recorded in E-038; exact hashes in `SHA256SUMS`.
- **Failure/correction/uncertainty:** This is author evidence, not independent verification. The preflight is candidate-side code and cannot bind a malicious administrator or replace host-enforced review. The `pull_request` build is deliberately excluded from the preflight because it checks out a synthetic merge ref where local/remote identity and the parent chain cannot hold. Hosted PR checks on PR #16 remain **Failed** for the structural whole-branch-scope reason now recorded in GATE-STATUS; Playwright E2E remains **Failed** for the pre-existing environmental reason. G2-G04 host-authenticated reviewer identity remains Open and blocking.
- **Resulting status/gate change:** G2-G02 publication enforcement is author Self-check passed and wired. G2 remains `Self-check passed`; `SAFE TO SWITCH: NO`. No acceptance, independent audit, merge, deployment, PR #16 edit, or G3-A controller change occurred.
- **Exact next action:** Continue G3-A external controller provisioning per CONTROL-STATE `first_unfinished_action`; this checkpoint does not advance that work.

### CL-0060 — 2026-09-22 UTC — hosted execution of the wired preflight observed and recorded

- **Actor:** Claude Code G2 continuation author (same author lineage as CL-0059; not an independent reviewer).
- **Task/phase:** G2 autonomy correction / recording observed hosted results for the checkpoint published as CL-0059.
- **Repository/branch/starting commit:** Existing `divergence/reliability-staging` at `43005abf53ad9aca1d3fb4dbf8ec32a6dbf0f9c7`.
- **Authority/source:** Requirement #8 of the user's G2 directive — every required hosted check is recorded as observed Failed or Open, never described as expected success.
- **Action and affected files:** Read the hosted job logs for both runs triggered by CL-0059 and recorded exact results in `GATE-STATUS.md`, `EVIDENCE-INDEX.md` (E-053), and `CURRENT-TASK.md`. Corrected the out-of-scope path list, which had omitted `docs/DIVERGENT-USER-INTERFACE-OPTIMIZER-SPEC.md`.
- **Reason and rejected alternatives:** CL-0059 asserted the preflight was wired without hosted proof that the new step runs in the runner, and its PR-failure row cited only a local reproduction. Primary logs now exist, so the record is upgraded from author self-check to observed. Rejected leaving the claim unproven, and rejected describing the PR failure as anything other than Failed.
- **Command/test/check and actual result:** Push run `35746157026` (run 81): all six steps succeeded; step 6 `Verify publication preflight` executed 15:16:32Z→15:16:33Z rather than being skipped; gate tests reported 65/65 in the runner. Pull_request run `35746165430`, job `106808232633`: **failed** at `Enforce the active task` for a stale base plus nine out-of-scope paths and the Open host-authenticated-reviewer requirement, with the preflight step correctly skipped. The preceding push run `34419392732` for `2a6434d` had **failed**, consistent with the stale confirmed-checkpoint record CL-0059 repaired.
- **Evidence/artifact/hash:** E-053 (upgraded); run IDs above; exact hashes in `SHA256SUMS`.
