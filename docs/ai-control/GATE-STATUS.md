...
| Signed-webhook transport correction | Self-check passed | Live Vercel request evidence and official runtime documentation identified parsed/consumed request bytes. The Web-standard correction is deployed READY as `dpl_7igFACJPxHH9K19NrLG5AaR6C4rN` at exact digest `675a965131519111495e877e7d3b95502d213637`. Bootstrap idempotency, post-bootstrap health, and invalid-signature rejection passed live. A later records publication at `b9af1b0…` corrupted the ledger and still lacked genuine signed-delivery proof; it remains in history while the current tree restores the trusted append-only ledger and adds safe diagnostic logging plus valid/duplicate/invalid hostile tests. Genuine signed and authenticated duplicate-redelivery proof remain Open. |
