...
| Signed-webhook transport correction | Self-check passed | Forward-only recovery `4d76dbea…` retains malformed `b9af1b0…` in history, restores the trusted append-only ledger, and adds safe diagnostic logging plus valid/duplicate/invalid hostile tests. Exact controller digest `b8a044d14415d97cfe41853548d3ef50d61e00d9` is deployed READY as `dpl_8N5fdgtDovjrrjyHf8e6BubmqALM`; stable health confirms that digest, durable initialization, observe mode, and inactive enforcement. Genuine signed and authenticated duplicate-redelivery proof remain Open. |
| Login-free webhook-secret recovery | Self-check passed | Trigger `269afa2b…` produced genuine Hookshot push delivery `4010aa1e-ac0b-11f1-9a8b-b30352f0b612`; safe logs prove a nonempty signed JSON body reached the exact deployment but failed signature verification, establishing a GitHub/Vercel secret mismatch. A BOOTSTRAP_SECRET-authenticated maintenance endpoint uses the App private key already held by Vercel to update GitHub through `/app/hook/config` and request App-authenticated redelivery; 14/14 package tests and 35/35 combined G3-A tests pass. Publication, deployment, synchronization, and live proof remain Open. |
| Live worker dispatch | Open — BLOCKING | `AUTHOR_WORKER_URL` and `AUDITOR_WORKER_URL` are unconfigured in the live controller project. Local worker tests do not prove autonomous live author or auditor execution. |
...
|---|---|---|
| AI Course Control — push to `divergence/reliability-staging` | **Observed success** | Hosted run `35746157026` (run 81) for `43005abf53ad9aca1d3fb4dbf8ec32a6dbf0f9c7`: all six steps succeeded, including step 6 `Verify publication preflight`, which executed (15:16:32Z→15:16:33Z) rather than being skipped. Gate tests reported 65/65 in the runner. For contrast, the preceding push run `34419392732` for `2a6434d` **failed** — consistent with the stale confirmed-checkpoint record this checkpoint repairs. |
| AI Course Control — pull_request on PR #16 | **Failed** | Hosted run `35746165430`, job `106808232633`, for merge ref `8158da12a26c835aa8726ea098575ab13404465d` against base `0c6444945cd814bb67ee5f437785d106458383c1`; reproduced locally with the same PR semantics. The runner rejected it for a stale base plus nine out-of-scope paths and the still-Open host-authenticated-reviewer requirement. In that same run the publication-preflight step was correctly **skipped**, confirming the `push`-only guard. PR #16 aggregates the whole staging branch, so B0/F0/G1 artifacts (`02-MASTER-SYSTEM-AND-REQUIREMENT-BLUEPRINT.md`, `docs/reliability/f0/**`, `.github/copilot-instructions.md`, `DIVERGENCE-G1-CONTROL-UPGRADE-HANDOFF.md`, `docs/DIVERGENT-USER-INTERFACE-OPTIMIZER-SPEC.md`) fall outside the active G2 profile's `allowed_paths`. Structural and permanent while PR #16 remains a whole-branch pull request; it is correct policy enforcement, not a defect to suppress. |
| Playwright E2E suite | **Failed** | **Cause corrected (E-054).** Not a missing browser: Chromium runs and the suite executes — hosted job `106808204898` reports **28 passed, 15 failed, 2 skipped**. The failures are application-level assertions against the approved shell: `html[data-layout]` resolves to empty where `"gold"` is expected, and `Talk to AI` and `.screen-trash` never appear before timeout. Pre-existing and not attributable to this work: the Sep 10 run on `2a6434d` (job `102691312025`, twelve days and many commits earlier) produced a 21,831,380-byte / 52-file report against this run's 21,828,649-byte / 52-file report, and no application file changed in this checkpoint. The earlier "no browser available" wording came from E-020/E-023/E-024, which describe a genuinely different and now-stale failure mode. Repairing these tests is product work and remains outside G2 scope. |
