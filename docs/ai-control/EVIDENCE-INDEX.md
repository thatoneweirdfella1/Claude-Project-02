...
| E-010 | `scripts/ai-course-control.test.mjs`, local Node output, and GitHub Actions run | Course controller accepts an in-scope checkpoint and rejects wrong repository/branch, protected branch, scope drift, deletion, immutable-baseline changes, ledger rewrites, missing records, bad hashes/statuses, unsupported independent claims, and locked-task activation | Self-check passed: 17/17 local tests and GitHub workflow success | Commit `d417f10cd3ee543fb0facde7bd620b0a029ebd72`; run `34066339481` | Repository ruleset and cold-start independent trial remain Open. |
