# ACCOUNT 2 REMAINING SITE REPAIR — BINDING WORK ORDER

Status: ACTIVE AND BINDING
Repository: `thatoneweirdfella1/Claude-Project-02`
Only writable branch: `account2/layer-8-candidate-v1`
...
9. Write `docs/checkpoints/ACCOUNT2-L8-GROUP-<N>.md` containing full commit SHA, changed files, tests, build result, evidence, unresolved rows, and exact next row.
...
- A final `docs/checkpoints/ACCOUNT2-L8-FINAL.md` lists commits, changed files, fixed rows, blocked rows, tests, build, preview identity if any, remaining authorization gates, and exact handoff for independent adoption.
