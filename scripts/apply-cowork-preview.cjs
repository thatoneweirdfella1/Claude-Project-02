...
const texts = partNames.map((name, index) => {
  const source = fs.readFileSync(path.join(partsDir, name), "utf8");
  const hash = crypto.createHash("sha256").update(source).digest("hex");
  if (hash !== hashes[index]) throw new Error(`Cowork overlay part mismatch: ${name}`);
  return source;
});

const archive = Buffer.from(texts.join("").replace(/\s+/g, ""), "base64");
...
const repairedSourcePaths = [
  ...
  "src/services/modelRegistry.test.ts",
  ...
  "src/components/methodology/MethodologyDropdown.tsx",
  ...
  "src/components/detection/StateDetectionStatusBar.tsx",
...
const saved = new Map(repairedSourcePaths.map((relativePath) => [
  relativePath,
  fs.readFileSync(path.join(root, relativePath), "utf8"),
]));
const temporaryArchive = path.join(root, ".cowork-delta.tar.gz");
fs.writeFileSync(temporaryArchive, archive);
try {
  cp.execFileSync("tar", ["-xzf", temporaryArchive, "-C", root], { stdio: "inherit" });
} finally {
  if (fs.existsSync(temporaryArchive)) fs.unlinkSync(temporaryArchive);
}
for (const [relativePath, source] of saved) {
...
for (const [relativePath, before, after] of fixes) {
  const file = path.join(root, relativePath);
  const source = fs.readFileSync(file, "utf8");
  if (!source.includes(before)) {
    if (!source.includes(after)) throw new Error(`Expected NodeNext import pattern missing in ${relativePath}`);
    continue;
  }
  fs.writeFileSync(file, source.replace(before, after));
}

const css = path.join(root, "src/styles/frozen-reference.css");
let cssSource = fs.readFileSync(css, "utf8");
cssSource = cssSource
  .replace(
  ...
  .replace(
    "height:96px !important; min-height:76px; max-height:132px; padding:10px 12px;",
    "height:122px !important; min-height:122px; max-height:122px; padding:10px 12px 32px; overflow-y:auto;",
  );
if (cssSource.includes("height: 158px !important") || cssSource.includes("height:96px !important")) {
  throw new Error("Frozen composer textarea override normalization failed");
}
fs.writeFileSync(css, cssSource);
...
console.log(
  `Applied complete Cowork preview overlay (${actual}), preserved ${saved.size} post-overlay repair files, normalized Vercel-only NodeNext imports, normalized the fixed composer input budget, and applied the preview All Tools stacking hotfix`,
);
