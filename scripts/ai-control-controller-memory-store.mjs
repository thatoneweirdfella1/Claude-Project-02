...
export class MemoryControllerStore {
  constructor(state) { this.state = structuredClone(state); this.leases = new Map(); this.events = []; this.attestations = []; this.queue = []; }
  ...
  async createCorrection(attestation) {
    ...
    task.attempt += 1;
    return { correction_id: `${attestation.task}:${task.attempt}`, task: attestation.task, attempt: task.attempt, failed_attestation_id: attestation.attestation_id, failed_candidate_sha: attestation.candidate_sha, defects: attestation.defects, acceptance_tests: attestation.acceptance_tests || attestation.defects, state: "Open" };
  }
