...
**Backfilled at Step 1.3, not produced at Step 0.** Step 0 marked this deliverable done in
BUILD-LOG.md's STEPS checklist, but the file itself was never written or saved anywhere in the
project tree. Step 1.3 requires it to name which texture file is which, so it's written here,
now, as a catch-up — not a retroactive claim that Step 0 produced it. See BUILD-LOG.md DECISIONS
for the same note.
