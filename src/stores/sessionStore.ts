...
interface SessionActions {
  ...
  setStateDetectionMode: (stateDetectionMode: StateDetectionMode) => void;
...
export const useSessionStore = create<SessionStore>((set) => ({
  ...
  loadSessionRecord: (record) =>
    set({
      ...
      maxRequestCost: 0,
      stateDetectionMode: record.stateDetectionMode ?? "manual-free",
  ...
  hydrate: (state) => set((current) => ({
    ...
    ...(state.stateDetectionMode !== undefined
      ? { stateDetectionMode: (["manual-free", "manual-paid", "automatic-paid"] as StateDetectionMode[]).includes(state.stateDetectionMode)
          ? state.stateDetectionMode
          : "manual-free" }
      : {}),
