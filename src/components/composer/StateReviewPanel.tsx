...
export function StateReviewPanel({ initial, onAccept, onKeepCurrent, onDismiss, onCancel }: StateReviewPanelProps) {
  ...
  return <div className="workflow-dialog" role="dialog" aria-modal="true" aria-labelledby="state-review-title">
    <div className="workflow-dialog__card surface-smoked-glass">
      <header><h2 id="state-review-title">State Detection</h2><p>Private local read. Nothing was sent to an AI.</p></header>
      ...
      <footer className="workflow-dialog__actions">
        ...
        <button type="button" onClick={onDismiss}>Dismiss reading</button>
        <button type="button" onClick={onKeepCurrent}>Keep current settings</button>
        <button type="button" className="primary" onClick={() => onAccept(result)}>Accept & Continue</button>
      </footer>
    </div>
  </div>;
}
