...
function TokenUsageContent() {
  ...
  const inputTokens = messageCount * 37;
  const outputTokens = messageCount * 54;
  return (
    <div className="accordion-panel__content accordion-stats">
      ...
      <div><span>Total Sessions Today</span><strong>{sessions.length}</strong></div>
    </div>
  );
}
...
export function AccordionStack() {
  ...
  const panels: PanelDefinition[] = [
    ...
    { key: "contextSnapshot", label: "Context Snapshot", icon: <Activity size={18} />, content: <ContextSnapshotContent /> },
